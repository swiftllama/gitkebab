...
define_basic_variables "libgit2-1.8.4" "android" "arm64" "debug"
...
# Cross-compiled libssh2.pc / openssl.pc / zlib.pc / pcre.pc live
# under the per-arch build dirs; feed them to pkg-config so libgit2's
# SelectSSH.cmake actually discovers libssh2 when we pass
# -DUSE_SSH=libssh2. (Without this it falls back to find_package which
# fails for cross builds, then to FATAL_ERROR "LIBSSH2 not found", or
# silently leaves SSH off depending on the version.)
