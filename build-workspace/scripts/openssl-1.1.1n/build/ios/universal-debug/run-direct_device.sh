...
CROSS_TOP="${IOS_SDKS_FOLDER}" CROSS_SDK="${IOS_SDK}" CROSS_COMPILE="${XCRUN_FOLDER}" CFLAGS="-arch armv7 -arch arm64" ${RELATIVE_SOURCE}/Configure ios-cross no-shared no-dso no-hw no-engine --prefix=${ROOT}/${BUILD_FOLDER} --openssldir=${ROOT}/${BUILD_FOLDER}
make clean
make
make install
