...
CROSS_COMPILE="${XCRUN_FOLDER}" CFLAGS="-arch i386 -arch x86_64 -isysroot ${IOS_SDKS_FOLDER}/${IOS_SDK}" ${RELATIVE_SOURCE}/Configure iossimulator-xcrun no-shared no-dso no-hw no-engine --prefix=${ROOT}/${BUILD_FOLDER} --openssldir=${ROOT}/${BUILD_FOLDER}
make clean
make
make install
