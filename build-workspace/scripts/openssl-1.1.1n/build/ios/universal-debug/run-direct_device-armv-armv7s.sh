...
CROSS_COMPILE="${XCRUN_FOLDER}" CFLAGS="-arch armv7 -arch armv7s -isysroot ${IOS_SDKS_FOLDER}/${IOS_SDK} -fembed-bitcode" ${RELATIVE_SOURCE}/Configure ios-xcrun no-shared no-dso no-hw no-engine --prefix=${ROOT}/${BUILD_FOLDER} --openssldir=${ROOT}/${BUILD_FOLDER}
make clean
make
make install
