...
autoreconf -if ${RELATIVE_SOURCE}
CFLAGS="-O3 -arch armv7 -arch armv7s -arch arm64 -isysroot $IOS_SDK -mios-version-min=${IOS_TARGET_VERSION} -fembed-bitcode -pipe -no-cpp-precomp" ${RELATIVE_SOURCE}/configure --prefix=${ROOT}/${BUILD_FOLDER}_device --with-crypto=openssl --with-libssl-prefix=${ROOT}/${OPENSSL_DIR} --host=armv7-apple-darwin --disable-debug --disable-dependency-tracking --disable-silent-rules --disable-examples-build --with-libz --with-libz-prefix=-${ZLIB_DIR} --disable-shared --enable-static 
make clean
make VERBOSE=1
make install
