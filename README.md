...
# Gitkebab - A libgit2 wrapper in C with Dart bindings


## Project layout

The project is currently laid out like so:
    ...
    │   ├── example       # c-based example executable
    │   ├── lib           # c code for libgitkebab.so and libgitkebab_static.a
    │   └── test          # c-based tests (using cmocka)
    │
    ├── build-workspace   # folder for cloning and building
    │   ├── build         # destination folder for produced build products
    │   ├── execution     # log folder used by state-chart-executor
    │   ├── patches       # patches to source code repos
    │   ├── README.md
    │   ├── scripts       # scripts used to clone/build
    │   └── source        # checked out source code of dependencies and gitkebab
    └── test-staging      # staging directory for tests

The `src` folder is laid out like so:

    src/
    ├── dart
    │   ├── conf-build-workspace.yaml     # ffigen conf file for generating dart bindings
    │   ├── copy-latest-gitkebab-build    # ad-hoc script to copy latest build from build-workspace to ./gitkebab-linx-debug
    │   ├── gitkebab-linux-debug          # copy of latest build from build-workspace, used for in-place evaluation of dart tests
    │   ├── lib                           # dart gitkebab library 
    │   │   ├── gitkebab.dart             # dart main library source (imports from src, exports)
    │   │   ├── src                       # dart library internal sources
    │   │   └── test                      # dart test sources
    │   ├── pubspec.yaml                  # pubspec used with ffigen to generate dart bindings
    │   └── test-staging                  # test staging folder for dart tests
    │
    ...
    └── test                              # c-based gitkebab tests (uses cmocka)
        ...
        ├── fixtures
        ├── test_session_progress.c
        │   ...
        └── test_status.c

For more information on the `build-workspace` layout see [build-workspace/README](build-workspace/README.md)


## Gitkebab C architecture

### Source code layout

At a file level, in `src/lib`:
...
operations. Its indended use is like so:
    ...
    gk_session *session = gk_session_new("git@gitea.ptskl.com:volund/experimental-notebook.git", GK_REPOSITORY_SOURCE_URL_SSH, "/tmp/clone1", "git", &session_progress_callback, &session_state_callback, &merge_conflict_query_callback);
